...
from bytecode.operators import (
    BINOPS_TO_OPERATORS,
...
class Terminator(Stmt):
  ...
  A terminator must either: - exit the function - jump to a block
...
class StaticRaise(Terminator):
  ...
  Note that if *exc_class* is None, a bare "raise" statement is implied (i.e.
...
class Var(EqualityCheckMixin, AbstractRHS):
  ...
  def versioned_names(self):
    """Known versioned names for this variable, i.e.

    known variable names in the scope that have been formed from applying SSA to
    this variable
    """
...
class Scope(EqualityCheckMixin):
  ...
  def get_exact(self, name):
    ...
    The returned variable has the exact name (exact variable version).
    """
    try:
...
class FunctionIR(object):
  ...
  def equal_ir(self, other):
    ...
    Equality is defined by being equal in fundamental structure (blocks, labels,
    IR node type and the order in which they are defined) and the IR nodes being
    equal. IR node equality essentially comes down to ensuring a node's
    `.__dict__` or `.__slots__` is equal, with the exception of ignoring 'loc'
    and 'scope' entries. The upshot is that the comparison is essentially
    location and scope invariant, but otherwise behaves as unsurprisingly as
    possible.
    """
    if type(self) is type(other):
  ...
  def get_definition(self, value, lhs_only=False):
    ...
    A Expr instance is returned by default, but if lhs_only is set to True, the
    left-hand-side variable is returned instead.
    """
    lhs = value
    while True:
  ...
  def get_assignee(self, rhs_value, in_blocks=None):
    """Finds the assignee for a given RHS value.

    If in_blocks is given the search will be limited to the specified blocks.
    """
