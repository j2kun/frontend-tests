"""Bytecode interpreter for converting bytecode to SSA.

Ported from numba/core/interpreter.py
"""
...
from numba.core import config, errors, ir
from numba.core.byteflow import AdaptCFA, AdaptDFA, BlockKind, Flow
...
class Assigner(object):
  ...
  def assign(self, srcvar, destvar):
    """Assign *srcvar* to *destvar*.

    Return either *srcvar* or a possible simplified assignment source (earlier
    assigned to *srcvar*).
    """
    srcname = srcvar.name
...
def _remove_assignment_definition(old_body, idx, func_ir, already_deleted_defs):
  """Deletes the definition defined for old_body at index idx from func_ir.

  We assume this stmt will be deleted from new_body.
...
def _call_function_ex_replace_kws_small(
  ...
  """Extracts the kws args passed as varkwarg for CALL_FUNCTION_EX.

  This pass is taken when n_kws <= 15 and the bytecode looks like:
...
def _call_function_ex_replace_args_small(
  ...
  """Extracts the args passed as vararg for CALL_FUNCTION_EX.

  This pass is taken when n_args <= 30 and the bytecode looks like:
...
class Interpreter(object):
  ...
  def _inject_call(self, func, gv_name, res_name=None):
    """A helper function to inject a call to *func* which is a python function.
  ...
  def op_RERAISE(self, inst, exc):
    ...
    if tryblk is not None:
      ...
      msg = (
          "Unreachable condition reached (op code RERAISE executed)"
