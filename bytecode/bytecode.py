"""This file adapted from numba/core/bytecode.py."""
...
class _ByteCode(object):
  ...
  def __init__(self, func_id):
    ...
    labels = set(x for x in dis.findlabels(code.co_code))
  ...
  def _compute_lineno(cls, table, code):
    ...
    This is mainly helpful because bytecode only includes line numbers for the
    first op involved in a line of source code.
    """
...
class ByteCodePy311(_ByteCode):
  ...
  def __init__(self, func_id):
    ...
    self.exception_entries = dis.Bytecode(func_id.code).exception_entries
...
class ByteCodePy312(ByteCodePy311):
  ...
  def __init__(self, func_id):
    ...
    entries = [e for e in dis.Bytecode(func_id.code).exception_entries]
...
class FunctionIdentity(object):
  ...
  def from_function(cls, func):
    ...
    code = func.__code__
    if not code:
      raise NotImplementedError('%s does not provide its bytecode' % func)
    ...
    self.arg_count = len(signature.parameters)
    self.arg_names = list(signature.parameters)
