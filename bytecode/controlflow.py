...
class CFGraph(object):
  ...
  def add_node(self, node):
    ...
    This is necessary before adding any edges from/to the node.  *node* can be
    any hashable object.
    """
  ...
  def process(self):
    """Compute essential properties of the control flow graph.

    The graph must have been fully populated, and its entry point specified.
    Other graph properties are computed on-demand.
    """
  ...
  def _find_back_edges(self, stats=None):
    ...
    An edge (src, dest) is a back edge if and only if *dest* dominates *src*.
    """
    ...
    for inst in self._iter_inst():
      ...
      if fn is not None:
        ...
        if inst.opname in {"SETUP_FINALLY"}:
          ...
          msg = "Use of unsupported opcode (%s) found at %s" % (
              inst.opname,
              loc.strformat(),
          )
